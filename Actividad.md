# Mariana Villanueva
## Grupo 1A - Fundamentos de Álgebra

**Ubica los siguientes números complejos en el plano**

**19)** -2 + 3i <br>
<img width="289" height="224" alt="image" src="https://github.com/user-attachments/assets/d5d69e8e-f4c2-42bf-88e8-4c529750aa5f" />

**20)** 1 - 2i <br> 
<img width="336" height="282" alt="image" src="https://github.com/user-attachments/assets/ab98b6d6-ab5b-497a-9474-a8ca6d81dfb9" />

**21)** -4 + 3i <br> 
<img width="395" height="272" alt="image" src="https://github.com/user-attachments/assets/36fdcdc1-7eab-4d02-867a-fc281760f921" />

**22)** 3 + i <br> 
<img width="373" height="287" alt="image" src="https://github.com/user-attachments/assets/a34722d6-9814-47d4-9a9f-75f6ff0c3454" />

**23)** -4 - 4i <br> 
<img width="343" height="239" alt="image" src="https://github.com/user-attachments/assets/3ca131e8-442d-46e6-b4e6-4444f7386d00" />

**24)** -2 - i <br>
<img width="346" height="254" alt="image" src="https://github.com/user-attachments/assets/ef011e09-d189-4dc4-b126-359be5b28a4f" />

**Resuelve las siguientes operaciones con los números complejos**

**25)** (-7 - 4i) - (2 + i) <br>
= -7 - 4i - 2 - i <br>
= -9 - 5i 
 
**26)** (2 - 4i) - (5 - 3i) <br>
= 2 - 4i - 5 + 3i <br>
= -3 - i

**27)** (7 - 8i) - (3i) - 7 <br>
= 7 - 8i - 3i - 7 <br>
=(7-7) + (-8i-3i) <br>
= -11i

**28)** (1 + 5i) + (-8 - 5i) + 3 <br>
= 1 +5i - 8 -5i + 3 <br>
= -4

**29)** -8 - (3 - 5i) + (4 + 8i) <br>
= -8 - 3 + 5i + 4 + 8i <br>
= -7 + 13i

**30)** (-4 + 2i) + (3i) + (-4 - 7i) <br>
= (-4 -4) + (2i + 31 - 7i) <br>
= -8 - 2i

**31)** (2i)(-4i) <br>
= $-8i^{2}$ <br>
= 8

**32)** $(-2i)(5i) \times 6$ <br>
$= (-10i^2)(6)$  
$= (10)(6) = 60$ <br>

**33)** $(-7i)(8 + 8i)(-2 - 8i)$ <br>
$= (-56i - 56i^2)(-2 - 8i)$ <br>
$= (-56i + 56)(-2 - 8i)$ <br>

<img width="243" height="126" alt="image" src="https://github.com/user-attachments/assets/bb233db1-527f-4bce-bc79-467b33413dea" /> <br>

$= -112 - 336i + 448i^2$ <br>
$= -112 - 336i - 448$ <br>
$= -560 - 336i$

**34)** 
$(-2 - 4i)(-1 - 6i)(7 - 6i)$ <br>
<img width="122" height="110" alt="image" src="https://github.com/user-attachments/assets/81742c34-49cf-4a66-9120-de7bb14c2df9" /> <br>

$= 2 + 16i + 24i^2$ <br>
= 2 + 16i -24 <br>
= -22 + 16i <br>
= (-22 + 16i)(7 - 6i) <br>
<img width="149" height="102" alt="image" src="https://github.com/user-attachments/assets/ecad1387-172d-436a-bc3a-9a9bb33b0f11" /> <br>

$= -154 + 244i - 96i^2$ <br>
$= -58 + 244i$ <br>

**35)** $(3i)(1 - 7i)(7 + 4i)$ <br>
$= (3i - 21i^2)(7 + 4i)$ <br>
$= (21 + 3i)(7 + 4i)$ <br>
 <img width="109" height="108" alt="image" src="https://github.com/user-attachments/assets/c2580ef9-f997-4f6a-81ce-a29d11c289f7" /> <br>

$= 147 + 105i - 12$ <br>
$= 135 + 105i$ <br><br>

**36)** $(-6i)(-5 + i)(6 - 2i)$ <br>
$= (30i - 6i^2)(6 - 2i)$ <br>
$= (6 + 30i)(6 - 2i)$ <br>
<img width="125" height="82" alt="image" src="https://github.com/user-attachments/assets/46e37b0f-1aa8-4b7f-b377-76d20eabdf32" /> <br>

$= 36 + 168i + 60$ <br>
$= 96 + 168i$ <br><br>

**37)** $\frac{10 - 7i}{1 + 3i}$ x $\frac{1 - 3i}{1 - 3i}$ <br>
= (10 - 7i) x (1 - 3i) <br>

<img width="86" height="61" alt="image" src="https://github.com/user-attachments/assets/15e6bc3d-01b8-40b2-85e2-3b064bd86a41" /> <br>
$= {10 - 37i + 21i^2}$ <br>
= 10 - 37i - 21 <br>
= -11 -37i <br>
= (1 + 3i) x (1 - 3i) <br>
<img width="88" height="64" alt="image" src="https://github.com/user-attachments/assets/96564f19-2ff0-48c6-a2c4-cfee5fc2400d" /> <br>

 $= {1 - 9i^2}$ <br>
 = 1 + 9 = 10 <br>
$= \frac{-11 - 37i}{10}$ <br>

= -1.1 - 3.7i <br>

**38)** $\frac{4 + 2i}{-1 - 10i}$ x $\frac{1 + 10i}{1 +10i}$ <br>

<img width="80" height="56" alt="image" src="https://github.com/user-attachments/assets/b69046be-ac37-4068-9ff9-df8882358352" /> <br>

$= {-4 + 38i + 20i^2}$ <br>

$= {-4 + 38i - 20}$ <br>

= -24 + 38i <br>

= (-1 - 10i) x (-1 + 10i) <br>
<img width="95" height="68" alt="image" src="https://github.com/user-attachments/assets/cf6f8f93-dc9c-4095-ba41-529db54bb2e1" /> <br>

 $= {1 - 100i^2}$ <br>
 = 1 + 100 = 101 <br>

 $= \frac{-24 + 38i}{101} = -0.23 + 0.37i$ <br>


**39)** $\frac{1 + 4i}{-1 - 6i}$ x $\frac{-1 + 6}{-1 + 6i}$ <br>

= (1 + 4i) x (-1 + 6i) <br>
<img width="61" height="53" alt="image" src="https://github.com/user-attachments/assets/3c3c0f0c-c538-401c-b303-a22ffd360bfd" /> <br>
$= {-1 + 2i + 24i^2}$ <br>
= -1 + 2i -24 <br>
= -25 + 2i <br>
= (-1 - 6i) x (-1 + 6i) <br>
<img width="62" height="44" alt="image" src="https://github.com/user-attachments/assets/e85df350-5835-4af3-9242-9371dabbedfc" /> <br>

$= {1 - 36^2}$ <br>
= 1 + 36 <br>
= 37 <br>

$= \frac{-25 + 2i}{37} = -0.68 + 0.05i$ <br>


**40)** $\frac{-8-4i}{1+i}$ x $\frac{1 - i}{1 - i}$ <br>

= (-8 - 4i) x (1 - i) <br>
<img width="80" height="67" alt="image" src="https://github.com/user-attachments/assets/943f7a5c-35cd-4383-a9af-f0705e29cb81" /> <br>

$= {-8 + 4i + 4i^2}$ <br>
= -8 + 12i - 4 <br>
= -12 + 4i <br>
= (1 + i) x (1 - i) <br>
<img width="94" height="59" alt="image" src="https://github.com/user-attachments/assets/f12b55cc-0b52-42bf-a81e-1794c6418dc5" /> <br>

$= {1 - i^2}$ <br>
= 1 + 1 <br>
= 2 <br>

= $\frac{-12 + 4i}{2}$ <br>

= -6 + 2i <br>

**41)** $\frac{-10+8i}{6+i}$ x $\frac{6 - i}{6 - i}$ <br>

= (-10 + 8i) x (6 - i) <br>
<img width="144" height="83" alt="image" src="https://github.com/user-attachments/assets/0e717ccb-b223-4fb0-af23-ee74ad1648a7" /> <br>

$= {-60 + 58i - 8i^2}$ <br>
= -60 + 58i + 8 <br>
= -52 + 58i <br>

= (6 + i) x (6 - i) <br>
<img width="111" height="80" alt="image" src="https://github.com/user-attachments/assets/4f6d05dd-0f81-43cb-9391-5a6c7ac0f94d" /> <br>

$= {36 - i^2}$ <br>
=  36+ 1 <br>
= 37 <br>

= $\frac{-52 + 58i}{37}$ <br>


= -1.41 + 1.57i <br>

 
**42)** $\frac{2-2i}{4-10i}$ x $\frac{4 + 10i}{4 + 10i}$ <br>

= (2 - 2i) x (4 + 10i) <br>
<img width="105" height="73" alt="image" src="https://github.com/user-attachments/assets/8c5f2461-f601-4b6e-ade3-4beb606c3cb5" /> <br>

$= {8 + 12i - 20^2}$ <br>
= 8 + 12i + 20 <br>
= 28 + 12i <br>

= (2 - 2i) x (2 + 2i) <br>
<img width="103" height="63" alt="image" src="https://github.com/user-attachments/assets/d0d3179c-6f12-4858-9577-ac8150a0c01b" /> <br>

$= {16 - 100i^2}$ <br>
=  16 + 100 <br>
= 116 <br>

= $\frac{28 + 12i}{116}$ <br>


= 0.24 + 0.10i <br>

**Calcula el valor absoluto de los siguientes números complejos**

**43)** $|-9 - 9i|$ <br>
$= \sqrt{(-9)^2 + (-9)^2}$ <br>
$= \sqrt{81 + 81}$ <br>
$= 12.7279$ <br><br>

**44)** $|8 - 6i|$ <br>
$= \sqrt{8^2 + (-6)^2}$ <br>
$= \sqrt{64 + 36}$ <br>
= 10 <br>

**45)** $|6 - 3i|$ <br>
$= \sqrt{36 + 9}$ <br>
$= 6.70$ <br><br>

**46)** $|10 + 10i|$ <br>
$= \sqrt{10^2 + 10^2}$ <br>
$= 14.1421$ <br>

**47)** $|6 - 10i|$ <br>
$= \sqrt{36 + 100}$ <br>
$= 11.6619$ <br><br>

**48)** $|-1 + 7i|$ <br>
$= \sqrt{(-1)^2 + 7^2}$ <br>
$= \sqrt{1 + 49}$ <br>
$= \sqrt{50}$
= 7.071

**Resuelve las siguientes potencias de $i$** <br><br>

**49)** $i^5$ <br>
$= \frac{5}{4}$ = 1 <br>
r=1 <br>
$= i^1 = i$ <br><br>

**50)** $i^{10}$ <br>
$= \frac{10}{4}$ = 2 <br>
r=2 <br> 
$= i^2 = -1$ <br><br>

**51)** $i^{20}$ <br>
$= \frac{20}{4}$ = 5 <br>
r=0 <br> 
$= i^5 = 1$ <br><br>

**52)** $i^{35}$ <br>
$= \frac{35}{4}$ = 8 <br>
r=3 <br> 
$= i^3 = -i$ <br><br>

**53)** $i^{256}$ <br>
$= \frac{256}{4}$ = 64 <br>
r=0 <br> 
$=i^0 = 1$ <br><br>

**54)** $i^{55}$ <br>
$= \frac{55}{4}$ = 13 <br>
r=3 <br> 
$= i^3 = -i$

**Convierte los siguientes números complejos a su forma polar**

**55)** $6 - 8i$ <br>
$r = \sqrt{(6)^2 + (-8)^2} = \sqrt{100} = 10$ <br>
$\theta = \tan^{-1}\left(\frac{-8}{6}\right) = -53.13^\circ + 360^\circ = 306.87^\circ$ <br>
$z = 10(\cos 306.87^\circ + i \sin 306.87^\circ)$ <br><br>

**56)** $5\sqrt{2} + 5\sqrt{2}i$ <br>
$r = \sqrt{(5\sqrt{2})^2 + (5\sqrt{2})^2} = \sqrt{50 + 50} = \sqrt{100} = 10$ <br>
$\theta = \tan^{-1}\left(\frac{5\sqrt{2}}{5\sqrt{2}}\right) = \tan^{-1}(1) = 45^\circ$ <br>
$z = 10(\cos 45^\circ + i \sin 45^\circ)$ <br><br>

**57)** $2 - 2\sqrt{3}i$ <br>
$r = \sqrt{(2)^2 + (-2\sqrt{3})^2} = \sqrt{4 + 12} = \sqrt{16} = 4$ <br>
$\theta = \tan^{-1}\left(\frac{-2\sqrt{3}}{2}\right) = -60^\circ + 360^\circ = 300^\circ$ <br>
$z = 4(\cos 300^\circ + i \sin 300^\circ)$ <br><br>

**58)** $\frac{3\sqrt{3}}{2} - \frac{3}{2}i$ <br>
$r = \sqrt{\left(\frac{3\sqrt{3}}{2}\right)^2 + \left(-\frac{3}{2}\right)^2} = \sqrt{\frac{27}{4} + \frac{9}{4}} = \sqrt{9} = 3$ <br>
$\theta = \tan^{-1}\left(\frac{-3/2}{\frac{3\sqrt{3}}{2}}\right) = -30^\circ + 360^\circ = 330^\circ$ <br>
$z = 3(\cos 330^\circ + i \sin 330^\circ)$ <br><br>

**59)** $-2i$ <br>
$r = \sqrt{0^2 + (-2)^2} = \sqrt{4} = 2$ <br>
$\theta = 270^\circ$ <br>
$z = 2(\cos 270^\circ + i \sin 270^\circ)$

**60)** $-7i$ <br>
$r = \sqrt{0^2 + (-7)^2} = \sqrt{49} = 7$ <br>
$\theta = 270^\circ$ <br>
$z = 7(\cos 270^\circ + i \sin 270^\circ)$

**Convierte los números complejos de su forma polar a su forma rectangular**

**61)** $\cos 30^\circ + i \sin 30^\circ$ <br>
$a = 1 \cos 30^\circ = \frac{\sqrt{3}}{2}$ <br>

$b = 1 \sin 30^\circ = \frac{1}{2}$ <br>

$z = \frac{\sqrt{3}}{2} + \frac{1}{2}i$ <br><br>

**62)** $2(\cos 60^\circ + i \sin 60^\circ)$ <br>
$a = 2 \cos 60^\circ = 1$ <br>
$b = 2 \sin 60^\circ = \sqrt{3}$ <br>
$z = 1 + \sqrt{3}i$ <br><br>

**63)** $1.5(\cos 90^\circ + i \sin 90^\circ)$ <br>
$a = 1.5 \cos 90^\circ = 0$ <br>
$b = 1.5 \sin 90^\circ = \frac{3}{2}$ <br>

$z = \frac{3}{2}i$ <br><br>

**64)** $2.5(\cos 120^\circ + i \sin 120^\circ)$ <br>
$a = 2.5 \cos 120^\circ = -\frac{5}{4}$ <br>

$b = 2.5 \sin 120^\circ = \frac{5\sqrt{3}}{4}$ <br>

$z = -\frac{5}{4} + \frac{5\sqrt{3}}{4}i$ <br><br>

**65)** $4(\cos 135^\circ + i \sin 135^\circ)$ <br>
$a = 4 \cos 135^\circ = -2\sqrt{2}$ <br>
$b = 4 \sin 135^\circ = 2\sqrt{2}$ <br>
$z = -2\sqrt{2} + 2\sqrt{2}i$ <br><br>

**66)** $3(\cos 180^\circ + i \sin 180^\circ)$ <br>
$a = 3 \cos 180^\circ = -3$ <br>
$b = 3 \sin 180^\circ = 0$ <br>
$z = -3$

**Obtén TODAS las raices de los siguientes complejos**

**67)** 2 raíces cuadradas de $4(\cos 30^\circ + i \sin 30^\circ)$ <br>
$r = \sqrt{4} = 2$, $n = 2$ <br><br>
Para $k = 0$: <br>
$\theta_1 = \frac{30^\circ + 360^\circ(0)}{2} = \frac{30^\circ}{2} = 15^\circ$ <br>
$z_1 = 2(\cos 15^\circ + i \sin 15^\circ)$ <br><br>
Para $k = 1$: <br>
$\theta_2 = \frac{30^\circ + 360^\circ(1)}{2} = \frac{390^\circ}{2} = 195^\circ$ <br>
$z_2 = 2(\cos 195^\circ + i \sin 195^\circ)$ <br><br>

**68)** 2 raíces cuadradas de $3(\cos 90^\circ + i \sin 90^\circ)$ <br>
$r = \sqrt{3}$, $n = 2$ <br><br>
Para $k = 0$: <br>
$\theta_1 = \frac{90^\circ + 360^\circ(0)}{2} = \frac{90^\circ}{2} = 45^\circ$ <br>
$z_1 = \sqrt{3}(\cos 45^\circ + i \sin 45^\circ)$ <br><br>
Para $k = 1$: <br>
$\theta_2 = \frac{90^\circ + 360^\circ(1)}{2} = \frac{450^\circ}{2} = 225^\circ$ <br>
$z_2 = \sqrt{3}(\cos 225^\circ + i \sin 225^\circ)$ <br><br>

**69)** 3 raíces cúbicas de $-4\sqrt{2} + 4i\sqrt{2}$ <br>
$n = 3$, $k = 0, 1, 2$ <br>
$r = \sqrt{(-4\sqrt{2})^2 + (4\sqrt{2})^2} = \sqrt{32 + 32} = \sqrt{64} = 8 \implies \sqrt[3]{8} = 2$ <br>
$\theta = 180^\circ - \tan^{-1}(1) = 180^\circ - 45^\circ = 135^\circ$ <br><br>
Para $k = 0$: <br>
$\theta = \frac{135^\circ + 360^\circ(0)}{3} = \frac{135^\circ}{3} = 45^\circ$ <br>
$w_0 = 2(\cos 45^\circ + i \sin 45^\circ)$ <br><br>
Para $k = 1$: <br>
$\theta = \frac{135^\circ + 360^\circ(1)}{3} = \frac{495^\circ}{3} = 165^\circ$ <br>
$w_1 = 2(\cos 165^\circ + i \sin 165^\circ)$ <br><br>
Para $k = 2$: <br>
$\theta = \frac{135^\circ + 360^\circ(2)}{3} = \frac{855^\circ}{3} = 285^\circ$ <br>
$w_2 = 2(\cos 285^\circ + i \sin 285^\circ)$ <br><br>

**71)** 5 raíces de $-32i$ <br>
$n = 5$ <br>
$r = \sqrt[5]{32} = 2$ <br>
$\theta = 270^\circ$ <br><br>
Para $k = 0$: <br>
$\theta = \frac{270^\circ + 360^\circ(0)}{5} = \frac{270^\circ}{5} = 54^\circ$ <br>
$w_0 = 2(\cos 54^\circ + i \sin 54^\circ)$ <br><br>
Para $k = 1$: <br>
$\theta = \frac{270^\circ + 360^\circ(1)}{5} = \frac{630^\circ}{5} = 126^\circ$ <br>
$w_1 = 2(\cos 126^\circ + i \sin 126^\circ)$ <br><br>
Para $k = 2$: <br>
$\theta = \frac{270^\circ + 360^\circ(2)}{5} = \frac{990^\circ}{5} = 198^\circ$ <br>
$w_2 = 2(\cos 198^\circ + i \sin 198^\circ)$ <br><br>
Para $k = 3$: <br>
$\theta = \frac{270^\circ + 360^\circ(3)}{5} = \frac{1350^\circ}{5} = 270^\circ$ <br>
$w_3 = 2(\cos 270^\circ + i \sin 270^\circ)$ <br><br>
Para $k = 4$: <br>
$\theta = \frac{270^\circ + 360^\circ(4)}{5} = \frac{1710^\circ}{5} = 342^\circ$ <br>
$w_4 = 2(\cos 342^\circ + i \sin 342^\circ)$

**72)** 6 raíces de 729 <br>
$n = 6$ <br>
$r = \sqrt[6]{729} = 3$ <br>
$\theta = 0^\circ$ <br><br>

Para $k = 0$: <br>
$\theta = \frac{0^\circ + 360^\circ(0)}{6} = \frac{0^\circ}{6} = 0^\circ$ <br>
$w_0 = 3(\cos 0^\circ + i \sin 0^\circ)$ <br><br>

Para $k = 1$: <br>
$\theta = \frac{0^\circ + 360^\circ(1)}{6} = \frac{360^\circ}{6} = 60^\circ$ <br>
$w_1 = 3(\cos 60^\circ + i \sin 60^\circ)$ <br><br>

Para $k = 2$: <br>
$\theta = \frac{0^\circ + 360^\circ(2)}{6} = \frac{720^\circ}{6} = 120^\circ$ <br>
$w_2 = 3(\cos 120^\circ + i \sin 120^\circ)$ <br><br>

Para $k = 3$: <br>
$\theta = \frac{0^\circ + 360^\circ(3)}{6} = \frac{1080^\circ}{6} = 180^\circ$ <br>
$w_3 = 3(\cos 180^\circ + i \sin 180^\circ)$ <br><br>

Para $k = 4$: <br>
$\theta = \frac{0^\circ + 360^\circ(4)}{6} = \frac{1440^\circ}{6} = 240^\circ$ <br>
$w_4 = 3(\cos 240^\circ + i \sin 240^\circ)$ <br><br>

Para $k = 5$: <br>
$\theta = \frac{0^\circ + 360^\circ(5)}{6} = \frac{1800^\circ}{6} = 300^\circ$ <br>
$w_5 = 3(\cos 300^\circ + i \sin 300^\circ)$

















